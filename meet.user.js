...
const DOM = window.trustedTypes?.createPolicy('BypassTT', { createHTML: HTML => HTML });
...
GM_registerMenuCommand(get_message("settings"), ()=>{
	...
	content.innerHTML = DOM.createHTML(`
	...
	window.addEventListener("keydown", function(e){
		if (e.keyCode == 17){ // CTRL
			if (!event.repeat){
				save.innerHTML = DOM.createHTML(get_message('resetBut'))
				save.onclick = _ => {
					let keys = GM_listValues()
					keys.forEach(key=>{
						GM_deleteValue(key)
	...
	window.addEventListener("keyup", function(e){
		if (e.keyCode == 17){ // CTRL
			save.innerHTML = DOM.createHTML(get_message('saveBut'))
			save.onclick = () => initSave()
		}
	})
	...
	if (db_get("volumeController", true)){
		if (!document.querySelector("#volumeController")){
			...
			function on_input_change(input_){
				...
				div.querySelector("span").innerHTML = DOM.createHTML(input_.value * 100);
			}
			...
			text.innerHTML = DOM.createHTML("100");
